...
from backend.schemas.user import LoginRequest, RegisterRequest, TokenResponse, UserInfo
...
async def login(req: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    # Keep legacy administrator credentials working, but prefer the hashed DB record.
    if username == settings.ADMIN_USERNAME and password == settings.ADMIN_PASSWORD:
        return TokenResponse(msg="Login successful", token=create_access_token(username, "admin"), role="admin", username=username)
    ...
    return TokenResponse(msg="Login successful", token=create_access_token(username, user.role), role=user.role, username=username)
